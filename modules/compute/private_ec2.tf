resource "aws_instance" "private-server" {
  ...
  tags = {
    Name        = lower("${var.vpc_name}-Prviate-Server-${count.index + 1}")
  ...
  user_data = <<-EOF
    ...
    sudo cp webhooktesting/scorekeeper.js /var/www/html/scorekeeper.js
  	#echo "<div><h1>${var.vpc_name}-Prviate-Server-${count.index + 1}</h1></div>" >> /var/www/html/index.nginx-debian.html
    sed -i '29i <center><div id="container"><h1>${var.vpc_name}-Prviate-Server-${count.index + 1}</h1></div></center>' /var/www/html/index.nginx-debian.html
  EOF
}